...
if __name__ == "__main__":
    ...
    def demo_connect_corner(N=6, config="A"):
        ...
        if config in ["A", "B"]:
            ...
            ports_A_TL = [
                Port(f"A_TL_{i}", (-d, a / 2 + i * sep), 0.5, 180)
                for i in range(N)
            ]
            ports_A_BR = [
            ...
            ports_A_BL = [
                Port(f"A_BL_{i}", (-d, -a / 2 - i * sep), 0.5, 180)
                for i in range(N)
            ]
            ...
            ports_B_BR = [
                Port(f"B_BR_{i}", (a / 2 + i * sep, -d), 0.5, 270)
                for i in range(N)
            ]
            ports_B_BL = [
                Port(f"B_BL_{i}", (-a / 2 - i * sep, -d), 0.5, 270)
                for i in range(N)
            ]
            ...
            ports_A_TL = [Port(f"A_TL_{i}", (-a, d + i * sep), 0.5, 180) for i in range(N)]
            ports_A_BR = [Port(f"A_BR_{i}", (a, -d - i * sep), 0.5, 0) for i in range(N)]
            ports_A_BL = [
            ...
            ports_B_TR = [Port(f"B_TR_{i}", (d + i * sep, a), 0.5, 90) for i in range(N)]
            ports_B_TL = [Port(f"B_TL_{i}", (-d - i * sep, a), 0.5, 90) for i in range(N)]
            ports_B_BR = [Port(f"B_BR_{i}", (d + i * sep, -a), 0.5, 270) for i in range(N)]
            ports_B_BL = [
